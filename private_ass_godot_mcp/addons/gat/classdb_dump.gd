...
## GAT ClassDB Dump — extracts complete engine schema to JSON.
##
## Usage: Open any Godot 4.6 project with this plugin enabled.
##        A button "Dump ClassDB" appears in the bottom panel.
##        Click it to generate data/engine_schema.json.

const OUTPUT_PATH := "res://addons/gat/engine_schema.json"
...
func _enter_tree() -> void:
	_button = Button.new()
	_button.text = "Dump ClassDB"
	_button.pressed.connect(_on_dump_pressed)
	add_control_to_bottom_panel(_button, "GAT")


func _exit_tree() -> void:
	if is_instance_valid(_button):
		_button.pressed.disconnect(_on_dump_pressed)
		remove_control_from_bottom_panel(_button)
		_button.queue_free()


func _on_dump_pressed() -> void:
	print("[GAT] Starting ClassDB dump...")
	...
	for class_name in sorted_classes:
		...
		if class_name.begins_with("@"):
			continue
		if class_name.begins_with("_") and not class_name.begins_with("__"):
			continue

		var class_data := _extract_class(class_name)
...
func _extract_class(class_name: String) -> Dictionary:
	var parent: String = ClassDB.get_parent_class(class_name)
	var properties: Array = _extract_properties(class_name)
	var methods: Array = _extract_methods(class_name)
	var signals: Array = _extract_signals(class_name)
	var constants: Array = _extract_constants(class_name)
	var enums: Array = _extract_enums(class_name)

	return {
		"class": class_name,
		"inherits": parent if parent != "" else null,
		"is_abstract": not ClassDB.can_instantiate(class_name),
		"is_singleton": Engine.has_singleton(class_name),
...
func _extract_properties(class_name: String) -> Array:
	var result: Array = []
	var list: Array = ClassDB.class_get_property_list(class_name)

	for prop in list:
		# Skip properties inherited from Object that every class has
...
func _extract_methods(class_name: String) -> Array:
	var result: Array = []
	var list: Array = ClassDB.class_get_method_list(class_name, true)  # true = include inherited

	# Deduplicate — only keep methods defined on this class (or unique to it)
	# To get only this class's methods, subtract parent's methods
	var parent: String = ClassDB.get_parent_class(class_name)
...
func _extract_signals(class_name: String) -> Array:
	var result: Array = []
	var list: Array = ClassDB.class_get_signal_list(class_name, true)

	# Only keep signals defined on this class
	var parent: String = ClassDB.get_parent_class(class_name)
...
func _extract_constants(class_name: String) -> Array:
	var result: Array = []
	var list: PackedStringArray = ClassDB.class_get_integer_constant_list(class_name, true)

	# Only keep constants defined on this class
	var parent: String = ClassDB.get_parent_class(class_name)
	...
	for const_name in list:
		if const_name in parent_constants:
			continue
		var value = ClassDB.class_get_integer_constant(class_name, const_name)
		result.append({
			"name": const_name,
			"value": value,
		})
...
func _extract_enums(class_name: String) -> Array:
	var result: Array = []
	var list: PackedStringArray = ClassDB.class_get_enum_list(class_name, true)

	# Only keep enums defined on this class
	var parent: String = ClassDB.get_parent_class(class_name)
	...
	for enum_name in list:
		if enum_name in parent_enums:
			continue
		var values: Array = []
		var value_list: PackedStringArray = ClassDB.class_get_enum_constants(class_name, enum_name, true)
		for v in value_list:
			values.append({
				"name": v,
				"value": ClassDB.class_get_integer_constant(class_name, v),
			})
		result.append({
			"name": enum_name,
			"values": values,
...
func _type_int_to_name(type_int: int) -> String:
	# Godot's Variant.Type enum values (partial, common ones)
	var types := {
		...
		24: "NodePath",  # duplicate key workaround
		28: "PackedInt32Array",
	}
